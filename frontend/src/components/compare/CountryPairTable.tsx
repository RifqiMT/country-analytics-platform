...
function CellValue({
  ...
  if (cell.value === null || Number.isNaN(cell.value)) {
    return <DataTableEmpty>Not reported</DataTableEmpty>;
  }

  const y = formatYoY(cell.yoyPct, cell.yoyBps, preferBpsForMetric(id));
  const yearNote =
    cell.dataYear != null && cell.dataYear !== snapshotYear ? `Data year ${cell.dataYear}` : undefined;

  return (
    <DataTableMetricValue
      value={formatCompareMetricValue(id, cell.value, unit)}
      delta={y.text !== "—" ? y.text : undefined}
      deltaClassName={y.text !== "—" ? yoYClass(y.tone) : undefined}
      title={yearNote}
    />
  );
}
...
export default function CountryPairTable({
  ...
  const table = (
    <DataTable wide>
        <DataTableHead>
          <DataTableRow>
            <SortableTh columnKey="metric" sortKey={sortKey} sortDir={sortDir} onSort={onSort} sticky className={DATA_TABLE_TH_LABEL}>
              Metric
            </SortableTh>
            <SortableTh columnKey="countryA" sortKey={sortKey} sortDir={sortDir} onSort={onSort} align="right" className={`min-w-[7.5rem] bg-sky-50/30 ${DATA_TABLE_TH_SORT_RIGHT}`}>
            ...
            <SortableTh columnKey="countryB" sortKey={sortKey} sortDir={sortDir} onSort={onSort} align="right" className={`min-w-[7.5rem] bg-amber-50/30 ${DATA_TABLE_TH_SORT_RIGHT}`}>
            ...
            <SortableTh columnKey="comparison" sortKey={sortKey} sortDir={sortDir} onSort={onSort} className={`min-w-[9rem] ${DATA_TABLE_TH_LABEL}`}>
              Comparison
            </SortableTh>
          </DataTableRow>
        </DataTableHead>
              ...
              {group.rows.map((row) => {
                ...
                return (
                  <DataTableRow key={row.id}>
                    ...
                    <DataTableCell accent="a" numeric highlight={aLeads}>
                      <CellValue
                        id={row.id}
                        unit={row.unit}
                        cell={row.countryA}
                    ...
                    <DataTableCell accent="b" numeric highlight={bLeads}>
                      <CellValue
                        id={row.id}
                        unit={row.unit}
                        cell={row.countryB}
                    ...
                    <DataTableCell className="max-w-[12rem]">
                      {comparison ? (
                        <p className="text-sm leading-snug text-slate-700" title={comparison.secondary ?? undefined}>
                          {comparison.primary}
                        </p>
                      ) : (
                        <DataTableEmpty />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DataTableShell
          wide
          scrollClassName={fullscreen ? "min-h-0 flex-1" : "max-h-[28rem] sm:max-h-[36rem]"}
          footer={
            <DataTableFooterBar
              count={sorted.length}
              label={sorted.length === 1 ? "metric" : "metrics"}
              wide
            />
          }
        >
